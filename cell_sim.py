...
class cell_wrapper:
  ...
  def metabolism(self, env, living_cells):
    ...
    aa_limiting_rate = env['max_consumption']['amino_acids'] / aa_consumption
    O2_limiting_rate = env['max_consumption']['dO2'] / O2_consumption
    glucose_limiting_rate = env['max_consumption']['glucose'] / glucose_consumption
    # Find smallest rate
    limiting_rate = min(aa_limiting_rate, 
                        ...
                        1)
    if limiting_rate < 0:
      print('Negative Limiting Rate!')
      pdb.set_trace()

    mass_transfer['amino_acids'] = \
      (limiting_rate+aa_limiting_rate)/2 * aa_consumption
    mass_transfer['dO2'] = \
      (limiting_rate+O2_limiting_rate)/2 * O2_consumption
    mass_transfer['glucose'] = \
      (limiting_rate+glucose_limiting_rate)/2 * glucose_consumption
    mass_transfer['dCO2'] = -mass_transfer['dO2'] * self.cp['respiratory_quotient']
      
    # Perhaps change this at some point...
    IGG_production = mass_transfer['amino_acids']*self.IGG_conversion_ratio
  ...
  def step(self, env):
    ...
    cells['mass_transfer'] = self.metabolism(env, living_cells)
